	...
	Location location=gps.getLocation();
	    ...
	    //发送广播
	      ...
	      final  SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//设置日期格式
			...
			intent.putExtra("time",location == null ? "" : df.format(location.getTime()) + "" );
		...
		System.out.println("GPS service已经发送广播");
	      //尝试在GPS service中保存数据，将位置信息保存在文件中
			...
			if (location != null) {
				...
				editor2.putString("gpsinfo", "经度："+location.getLatitude() + "纬度：" +location.getLongitude()+ "海拔：" +location.getAltitude()+ "m\n定位时间："
						+df.format(location.getTime()) );
				editor2.commit();
